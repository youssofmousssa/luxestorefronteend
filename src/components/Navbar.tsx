        ...
        <div className="flex items-center justify-between h-12 sm:h-16 lg:h-20">
          ...
          <Link to="/" className="flex items-center space-x-2 group" onClick={handleLinkClick}>
            <div className="relative">
              <Sparkles className="w-6 h-6 lg:w-8 lg:h-8 text-primary animate-pulse" />
              <div className="absolute inset-0 w-6 h-6 lg:w-8 lg:h-8 bg-primary/20 rounded-full blur-md group-hover:bg-primary/40 transition-colors duration-300" />
            </div>
            <div className="text-lg sm:text-xl lg:text-2xl font-bold text-gradient relative">
              LuxeStore
          ...
          <div className="hidden lg:flex items-center space-x-1 xl:space-x-2">
            {navItems.map((item) => (
              ...
              >
                <Link
                  to={item.path}
                  className={`nav-link-enhanced ${isActive(item.path) ? 'text-primary' : ''} flex items-center space-x-1`}
                ...
                {item.hasDropdown && item.dropdownItems && (
                  <div className={`absolute top-full left-0 mt-2 w-56 glass-card border border-white/15 rounded-xl shadow-2xl transition-all duration-300 transform origin-top ${
          ...
          <div className="flex items-center space-x-2 lg:space-x-3">
            ...
            <button className="xl:hidden glass-button-enhanced p-2 lg:p-3">
              <Search className="w-4 h-4 lg:w-5 lg:h-5 text-white" />
            </button>

            {/* Enhanced User Account */}
            <Link to="/login" className="glass-button-enhanced p-2 lg:p-3 group hover:text-primary transition-colors" onClick={handleLinkClick}>
              <User className="w-4 h-4 lg:w-5 lg:h-5 group-hover:scale-110 transition-transform" />
            </Link>

            {/* Enhanced Shopping Cart */}
            <button
              onClick={() => dispatch({ type: 'TOGGLE_CART' })}
              className="glass-button-enhanced p-2 lg:p-3 relative group hover:text-primary transition-colors"
            >
              <ShoppingCart className="w-4 h-4 lg:w-5 lg:h-5 group-hover:scale-110 transition-transform" />
              {state.itemCount > 0 && (
                <span className="cart-badge-enhanced">
                  {state.itemCount}
                </span>
              )}
            </button>
            ...
            <button
              ...
              className="lg:hidden glass-button-enhanced p-2"
            >
              {isMenuOpen ? (
